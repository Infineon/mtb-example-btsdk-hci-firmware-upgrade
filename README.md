# HCI Firmware Upgrade sample application

## Overview

This application demonstrates Device Firmware Upgrade via the HCI interface (MCU).

## Features demonstrated

- Firmware Upgrade
- HCI communiation with embedded application

## Instructions

To demonstrate the app, work through the following steps:

##### To perform regular firmware upgrade:
...
3. Use Terminal emulation tools like Tera Term. Open the peripheral UART  (usually named WICED Peripheral UART) to view the log/trace messages (115200 baud rate).
4. Use ClientControl application to open HCI port (usually named WICED HCI UART) connecting with kit (115200 baud rate).
5. Use ClientControl Firmware Download controls to select *.bin file in the application build folder for firmware upgrade.
6. Download file and observe trace messages ending with validation and reboot.
7. New upgraded firmware is run after reboot.

##### To perform Secure firmware upgrade:
...
5. Clean and rebuild the application, plug any of the supported Evaluation boards into your PC. Build and download the application (See the respective Kit User Guide).
6. Use Terminal emulation tools like Tera Term. Open the peripheral UART  (usually named WICED Peripheral UART) to view the log/trace messages (115200 baud rate).
7. Copy generated (app-name)\_(bsp).ota.bin file in (app-name)/build/(bsp)/Debug folder back to ecdsa256/bin/(OS) directory.
8. Sign the binary .ota.bin file with the public key using ecdsa\_sign application in ecdsa256/bin/(OS) folder. This will generate file (app-name)\_bsp.ota.bin.signed
    - ecdsa\_sign (app-name)\_(bsp).ota.bin
9. Use ClientControl application to open HCI port (usually named WICED HCI UART) connecting with kit (115200 baud rate).
10. Use ClientControl Firmware Download controls to select *.bin.signed file in the application build folder for firmware upgrade.
11. Download file and observe trace messages ending with validation and reboot.
12. New upgraded firmware is run after reboot.
## BTSTACK version
